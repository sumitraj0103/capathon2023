...
namespace Meditrack.Models
{
    public class AppConstant
    {
        ...
        public async static Task AddFlyoutMenusDetails()
        {
            ...
            var guestDashboardInfo = AppShell.Current.Items.Where(f => f.Route == nameof(GuestDashboardPage)).FirstOrDefault();
            if (guestDashboardInfo != null) AppShell.Current.Items.Remove(guestDashboardInfo);
            ...
            if (App.UserDetails.RoleID == (int)RoleDetails.Guest)
            {

                var flyoutItem = new FlyoutItem()
                {
                    Title = "Dashboard Board",
                    Route = nameof(GuestDashboardPage),
                    FlyoutDisplayOptions = FlyoutDisplayOptions.AsMultipleItems,
                    Items =
                     {
                               new ShellContent
                                {
                                    Icon = Icons.pills,
                                    Title = "Medications",
                                    ContentTemplate = new DataTemplate(typeof(GuestDashboardPage)),
                ...
                {
                    AppShell.Current.Items.Add(flyoutItem);
                    await Shell.Current.GoToAsync($"//{nameof(GuestDashboardPage)}");
                }
            }
        }
    }
